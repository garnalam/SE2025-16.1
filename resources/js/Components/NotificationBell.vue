...
// 3. Lắng nghe Realtime khi Component được load
onMounted(() => {
    fetchNotifications();
    ...
    // In ra console để kiểm tra xem có lấy được ID không
    console.log("Đang lắng nghe thông báo cho User ID:", userId);

    // Kênh mặc định của Laravel Notifications luôn có dạng này
    Echo.private(`App.Models.User.${userId}`)
        .notification((notification) => {
            ...
            notifications.value.unshift({
                id: notification.id,
                data: { 
                    title: notification.title || 'Thông báo mới', // Fallback nếu thiếu title
...
<template>
    <div class="relative">
        ...
        <div v-if="isOpen" class="absolute right-0 z-50 mt-2 w-80 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
    ...
    <div class="shrink-0 mr-3">
        <img v-if="notify.data.user_avatar" :src="notify.data.user_avatar" class="h-8 w-8 rounded-full object-cover">
