...
import { Link, useForm, usePage } from '@inertiajs/vue3'; // <--- Import Link
    ...
    <div class="border-t border-slate-800 pt-4 mt-4">
        ...
        <div class="flex justify-between items-center mb-4">
            <h4 class="text-xs font-bold text-slate-500 font-mono uppercase tracking-widest flex items-center gap-2">
                ...
                Tất cả bình luận ({{ post.parent_comments ? post.parent_comments.length : 0 }})
            </h4>
...
<div v-if="canComment" class="flex gap-3 bg-slate-900/50 p-3 rounded-xl border border-white/5">
    <Link :href="route('profile.public', $page.props.auth.user.id)">
        <img class="h-8 w-8 rounded-lg object-cover border border-slate-700 hover:border-cyan-500 transition" :src="$page.props.auth.user.profile_photo_url" :alt="$page.props.auth.user.name">
    </Link>
    <div class="flex-1">
